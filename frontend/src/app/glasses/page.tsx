...
export default async function GlassesPage({ searchParams }: { searchParams: { brand?: string } }) {
        ...
        <div className={styles.container}>
          <div className={styles.sidebar}>
            ...
            <div className={styles.brandList}>
              <Link href="/glasses" className={`${styles.brandFilter} ${!searchParams.brand ? styles.active : ''}`}>
                ALL BRANDS
              </Link>
              {brands.map((brand: any) => (
                <Link key={brand.id} href={`/glasses?brand=${brand.slug}`} className={`${styles.brandFilter} ${searchParams.brand === brand.slug ? styles.active : ''}`}>
                  {brand.name}
                </Link>
              ))}
            </div>
          ...
          <div className={styles.content}>
            <div className={styles.header}>
              <h1 className={styles.title}>{activeBrand ? activeBrand.name : 'GLASSES'}</h1>
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className={styles.empty}>
                <p>No glasses found.</p>
                <Link href="/glasses" className={styles.clearFilter}>Clear Filters</Link>
              </div>
            )}
          </div>
        </div>
